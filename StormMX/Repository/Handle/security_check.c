...
void interface_create_check_security (void)
{
    ...
    if (!security->is_absolute && !security->generic_acl.interface_create)
    {
...
void interface_destroy_check_security (void)
{
    p_security_t security;

    security = (p_security_t) object_reference_current->security_reference;
    
    if (!security->is_absolute && !security->generic_acl.interface_destroy)
    {
#   if defined (SECURITY_GENERATE_ACCESS_DENIED)
        THROW_SYSTEM (EXCEPTION_ACCESS_DENIED);
#   else
