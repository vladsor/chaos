...
void pit_mode_set (int channel, unsigned int digits, unsigned int mode)
{
    ...
    port_uint8_out (PIT_MODE_REGISTER, (channel | ACCESS_LOW_COUNTER_U8 |
                                    ACCESS_HIGH_COUNTER_U8 | PIT_MODE_3 |
                                    PIT_COUNTER_MODE_BINARY));
}
...
void pit_frequency_set (int channel, unsigned int frequency)
{
    ...
    DEBUG_ASSERT ((channel < 3), "Invalid channel.");
...
//    DEBUG_ASSERT (DEBUG_LEVEL_ERROR, (divisor < 4), "");
    ...
    port_uint8_out (PIT_COUNTER_REGISTER (channel), 
        LOW_8 ((uint16_t) divisor));

    port_uint8_out (PIT_COUNTER_REGISTER (channel), 
        HIGH_8 ((uint16_t) divisor));   
}
