...
return_t dma_init (int argc UNUSED, char *argv[] UNUSED)
{
    int i;
...
return_t dma_register (int dma_channel, void **buffer)
{
    DEBUG_PRINT (DEBUG_LEVEL_INFORMATIVE, 
        ...
        __FILE__, __FUNCTION__, dma_channel, buffer);
...
return_t dma_unregister (int dma_channel)
{
    DEBUG_PRINT (DEBUG_LEVEL_INFORMATIVE, 
        "%s::%s (channel=%u)\n",
        __FILE__, __FUNCTION__, dma_channel);
    ...
    bool autoinit)
{
    ...
    DEBUG_PRINT (DEBUG_LEVEL_INFORMATIVE, 
        "%s::%s (channel=%u, length=%u, operation=%u, transfer_mode=%u, "
            "autoinit=%u)\n",
        __FILE__, __FUNCTION__, channel, length, operation, transfer_mode, 
            autoinit);
