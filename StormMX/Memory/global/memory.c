...
return_t global_memory_allocate (void **pointer, unsigned int bytes)
{
#ifdef SLAB
    int index;
    slab_block_t *block;
...
*/
    ...
    return_value = physical_memory_allocate (&page, SIZE_IN_PAGES (bytes));
    (*pointer) = (void *) (page * PAGE_SIZE);
...
return_t global_memory_deallocate (void *pointer)
{
#ifdef SLAB
    slab_superblock_t *superblock = 
