...
//            sizeof (mmap->size)), index++)
        ...
        {
            mmap = ((multiboot_memory_map_t *) multiboot_memory_map) + index;
            
            DEBUG_PRINT (DEBUG_LEVEL_INFORMATIVE1, 
                "%s: Map[%i] = %p, size = 0x%04X, base_addr = %X,"
...
return_t physical_memory_init (int argc UNUSED, char *argv[] UNUSED) 
{
    ...
    DEBUG_PRINT (DEBUG_LEVEL_INFORMATIVE,
        "%s: Total memory: %u pages, %u KiB, %u MiB.\n", 
        DEBUG_MODULE_NAME, 
        physical_pages, physical_pages * PAGE_SIZE / KIB, 
        physical_pages * PAGE_SIZE / MIB);
    ...
    DEBUG_PRINT (DEBUG_LEVEL_INFORMATIVE,
        "%s: Free memory: %u pages, %u KiB, %u MiB.\n", 
        DEBUG_MODULE_NAME, 
