...
#   define DEBUG_SUPPLIER REFERENCE_NULL
#endif
...
#define DEBUG_PRINT(level, message...) \
    do { \
        ...
        { \
            wchar_t __temp_buffer__[WSTRING_MAX_LENGTH]; \
            char __temp_buffer2__[STRING_MAX_LENGTH]; \
            wchar_t __function_name__[WSTRING_MAX_LENGTH]; \
            wstring_print (__function_name__, L"%s", __FUNCTION__); \
            string_print (__temp_buffer2__, ## message); \
            string_print (__temp_buffer__, L"%s", __temp_buffer2__); \
