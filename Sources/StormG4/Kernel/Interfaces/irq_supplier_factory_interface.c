...
#define DEBUG_MODULE_NAME "IrqSupplierFactory"
    ...
    sequence_t parameters_in)
{
    ...
    create = (p_irq_supplier_factory_create_t) (address_t) function;

    irq_number = ((p_uint32_t) parameters_in.data)[0];
    
    create (context, event_supplier, irq_number);
}
...
interface_reference_t irq_supplier_factory_interface_register (
    p_irq_supplier_factory_interface_table_t table)
{
    ...
    interface_description = interface_description_register (
        INTERFACE_IRQ_SUPPLIER_FACTORY_ID,
        L"irq_supplier_factory",
