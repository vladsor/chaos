...
return_t system_debug_init (int argc, char *argv[], char **envp UNUSED)
{
    int i;
...
return_t system_debug_print (uint8_t level, const char *module_name UNUSED, 
    const char *function_name UNUSED, uint32_t line UNUSED, const char *string)
{
    unsigned int index;
...
return_t system_debug_printw (uint8_t level, const wchar_t *module_name UNUSED,
    const char *function_name UNUSED, uint32_t line UNUSED, 
    const wchar_t *string)
{
    ...
    for (index = 0; string[index] != L'\0'; index++)
    {
        ...
        {
            put_character (x_position, y_position, (char) string[index],
                text_attribute);
            x_position++;
