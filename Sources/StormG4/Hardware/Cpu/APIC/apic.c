...
void apic_connect_bsp (void)
{
	if (pic_mode) 
    {
		...
		port_uint8_out (0x22, 0x70);
		port_uint8_out (0x23, 0x01);
	}
}

void apic_disconnect_bsp (void)
{
	if (pic_mode) 
	{
		/*
		...
		port_uint8_out (0x22, 0x70);
		port_uint8_out (0x23, 0x00);
	}
}

void apic_disable_local (void)
{
	unsigned long value;
...
void sync_Arb_IDs (void)
{
	...
	DEBUG_PRINT (DEBUG_LEVEL_INFORMATIVE, "Synchronizing Arb IDs.\n");
...
void  apic_setup_local (void)
{
    ...
    {		/* !82489DX */
		...
		DEBUG_PRINT (DEBUG_LEVEL_INFORMATIVE,
            "ESR value before enabling vector: %08lx\n", 
            value);
		...
		DEBUG_PRINT (DEBUG_LEVEL_INFORMATIVE,
            "ESR value after enabling vector: %08lx\n", 
            value);
	} 
    else 
    {
...
static int detect_init_APIC (void)
{
	u32 h, l, features;
	...
	DEBUG_PRINT (DEBUG_LEVEL_INFORMATIVE, "Found and enabled local APIC!\n");
...
static unsigned int get_8254_timer_count(void)
{
	...
	port_uint8_out_pause (0x43, 0x00);
	count = port_uint8_in_pause (0x40);
	count |= port_uint8_in_pause (0x40) << 8;
...
void apic_setup_LVTT (unsigned int clocks)
{
...
void apic_setup_timer (void * data)
{
...
void apic_setup_clocks (void)
{
	...
	calibration_result = apic_calibrate_clock ();
	/*
	 * Now set up the timer for real.
	 */
	apic_setup_timer ((void *) calibration_result);
