...
return_t elf_parse (elf_header_t *elf_header, elf_parsed_t *elf_parsed)
{
        ...
        elf_parsed->symbol_header == NULL)
    {
        DEBUG_PRINT (DEBUG_LEVEL_ERROR, 
            "%s: Elf image don't have a string table and symbol table\n",
            DEBUG_MODULE_NAME);
...
return_t elf_resolve (elf_parsed_t *elf_parsed, function_info_t *function)
{
        ...
        symbol_header->size; symbol_index++)
    {
        ...
        else if (symbol->value == 0 && symbol->section_header == 0)
        {
            ...
            {
                DEBUG_PRINT (DEBUG_LEVEL_ERROR, 
                    "%s: Unresolved symbol in module: %s\n", 
                    DEBUG_MODULE_NAME,
                    name);
...
return_t elf_relocate (elf_parsed_t *elf_parsed)
{
             ...
             relocation_header->size; index++)
    {
        ...
        if (relocation[index].symbol_type == ELF_RELOCATION_386_32)
        {
            (*relocation_address) = (*relocation_address) + symbol_address;
        }
        else if (relocation[index].symbol_type == ELF_RELOCATION_386_PC32)
        {
...
return_t elf_load_module (elf_parsed_t *elf_parsed)
{
    ...
    return_value = physical_memory_allocate (&physical_memory_global, &pages, 
    ...
    for (index = 0; index < elf_header->section_header_entries; index++)
    {
            ...
            section_header->type == ELF_SECTION_TYPE_PROGRAM)
        {
            ...
            DEBUG_PRINT (DEBUG_LEVEL_INFORMATIVE, 
                ...
                DEBUG_MODULE_NAME, __FUNCTION__, 
                section_header, (void *) section_header->address, 
            ...
            if (section_header->address % PAGE_SIZE != 0)
            {
                ...
                unsigned int flags;
                page_number_t buffer_page_number;
...
//                virtual_memory_reserve (virtual_memory, 
//                    PAGE_NUMBER (section_header->address), 1, 
//                    &buffer_page_number, VIRTUAL_MEMORY_OPTION_ALLOW_OVERLAP);

                /* If this section overlaps another section, find the
                   physical addres of the page that has already been
                   mapped at this place. */

                return_value = virtual_memory_lookup (virtual_memory, 
                    PAGE_NUMBER (section_header->address), &buffer_page_number, 
                    &flags);
                ...
                else 
                {
                    ...
                    return_value = physical_memory_allocate (
                        &buffer_page_number, 1);
                    ...
                    return_value = virtual_memory_map (virtual_memory, 
                         PAGE_NUMBER (section_header->address),
                         buffer_page_number, 1, PAGE_USER);
                ...
                buffer = (void *) PAGE_ADDRESS (buffer_page_number);

                DEBUG_PRINT (DEBUG_LEVEL_INFORMATIVE, 
                    "%s: %s: Copying to %p from %p (%d bytes)\n",
                    DEBUG_MODULE_NAME, __FUNCTION__, 
...
//                virtual_memory_reserve (virtual_memory, 
//                    PAGE_NUMBER (section_header->address), 1, 
//                    &buffer_page_number, VIRTUAL_MEMORY_OPTION_MAP);
                
                return_value = virtual_memory_lookup (virtual_memory, 
                    PAGE_NUMBER (section_header->address) + page_offset, 
                    &buffer_page_number, &flags);
                ...
                else
                {
                    ...
                    // writable).
                    virtual_memory_reserve (virtual_memory,
                        PAGE_NUMBER (section_header->address) + page_offset, 1, 
                        &buffer_page_number, 0);
#if 0
...
return_t elf_get_entry_point (elf_parsed_program_t *elf_parsed, 
    address_t *address)
{
    (*address) = elf_parsed->elf_header->entry_point;
