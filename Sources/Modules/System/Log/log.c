/* $Id: log.c,v 1.10 2000/09/17 20:07:53 hal Exp $ */
...
return_type main (void)
{
  ...
  if (system_thread_create () == SYSTEM_RETURN_THREAD_NEW)
  {
    ...
    if (console_init (&console_structure_server, &empty_tag) != CONSOLE_RETURN_SUCCESS)
    {
  ...
  else
  {
    ...
    if (console_init (&console_structure_kernel, &empty_tag) != CONSOLE_RETURN_SUCCESS)
    {
      system_call_debug_print_simple ("Couldn't connect to console service.");
      return -1;
    }
