...
{
    uint32_t flags;
    uint32_t  drive_select;              /* ATA, ATAPI only (LUN for SCSI?) */
    uint32_t  multi_sectors;            /* ATA only */
    uint32_t sectors;
    uint32_t heads;
    uint32_t cylinders;/* CHS ATA only */
    uint32_t io_address;
...
typedef atapi_toc_t * p_atapi_toc_t;

//extern int InterruptOccured;
//extern int awaitInterrupt (uint16_t IRQMask, unsigned Timeout);
//extern void irqStart(void);
//extern void irqEnd(void);

extern bool ata_internal_probe (p_drive_t drive);
extern void ata_internal_command (p_drive_t drive, p_drive_command_t command, 
    uint8_t count, uint8_t command_byte);

extern int atapi_internal_command (p_drive_t drive, p_drive_command_t command, 
    uint8_t *packet);
    
extern bool ata_select (uint16_t io_base, uint8_t command);
extern bool ata_probe (void);
extern int ata_command (p_drive_t drive, p_drive_command_t command);

extern int atapi_command (p_drive_t drive, p_drive_command_t command);
extern int atapi_eject (p_drive_t drive, bool load);
extern int atapi_play (p_drive_t drive, uint32_t start, uint32_t end);
extern int atapi_toc_read (p_drive_t drive, p_atapi_toc_t contents, unsigned count);
extern int atapi_pause (p_drive_t drive, bool play);
