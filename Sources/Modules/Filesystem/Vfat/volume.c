    ...
    bool *has_fat)
{
    ...
    if (test_boot_sector->filesystem_type[0] != 0)
    {
	    ...
	    L"%S: test_boot_sector->filesystem_type: [%.8s]\n",
            DEBUG_MODULE_NAME,
    ...
    if (string_compare_max (test_boot_sector->filesystem_type, "FAT12", 5) == 0
        || 
        string_compare_max (test_boot_sector->filesystem_type, "FAT16", 5) == 0
	||     
        string_compare_max (
            ((boot_sector_32_t *) test_boot_sector)->filesystem_type, "FAT32", 
            5) == 0)
    {
	memory_deallocate (test_boot_sector);
 ...
 */
return_t vfat_volume_create_data (
    ...
    handle_reference_t device_to_mount)
{
    ...
    uint32_t root_directory_length, fat_size, total_sectors, 
        first_data_sector, data_sectors;
    ...
    return_value = vfat_read_sectors (device_to_mount, 0, 1, boot_sector);
    ...
    vfat_volume->storage_device = device_to_mount;
    ...
    vfat_volume->fat_entries_per_sector = boot_sector->bytes_per_sector / 32;
...
//      vfat_volume->total_volume_length = boot_sector->total_sectors_16;
            ...
            memory_clear (vfat_volume->label, 12);
            string_copy_max (vfat_volume->label, boot_sector->volume_label, 
                             11);
...
//      vfat_volume->total_volume_length = boot_sector->total_sectors_32;
            ...
            string_copy_max (vfat_volume->label,
                             ((boot_sector_32_t *) boot_sector)->volume_label, 
                             11);
            vfat_volume->label[12] = 0;
    ...
    memory_deallocate (boot_sector);

    switch (vfat_volume->fat_type)
    {
        case FAT_12:
        {
            vfat_volume->get_next_cluster = 
                &fat12_get_next_cluster;
  
            vfat_volume->find_available_cluster = 
                &fat12_find_available_cluster;

            vfat_volume->count_available_clusters = 
                &fat12_count_available_clusters;
  
            vfat_volume->write_cluster = 
                &fat12_write_cluster;
        ...
        case FAT_16:
        {
            ...
            vfat_volume->find_available_cluster = 
                &fat16_find_available_cluster;

            vfat_volume->count_available_clusters = 
                &fat16_count_available_clusters;
        ...
        case FAT_32:
        {
            ...
            vfat_volume->count_available_clusters = 
                &fat32_count_available_clusters;
...
void vfat_volume_add_file_data (
    p_vfat_volume_t vfat_volume UNUSED,
    p_vfat_file_t file UNUSED)
{
}

p_vfat_file_t vfat_volume_get_file_data (
    p_vfat_volume_t vfat_volume UNUSED,
    const wchar_t *file_name UNUSED)
{
    return NULL;
}

void vfat_volume_remove_file_data (
    p_vfat_volume_t vfat_volume UNUSED,
    p_vfat_file_t file UNUSED)
{
}
