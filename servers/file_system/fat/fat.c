/* $Id: fat.c,v 1.3 2001/03/11 21:34:09 jojo Exp $ */
...
static void handle_connection (ipc_structure_type *ipc_structure)
{
  ...
  fat_info.block_structure.output_mailbox_id = mailbox_id[1];
    ...
    switch (message_parameter.message_class)
    {
      ...
      case IPC_FILE_DIRECTORY_ENTRY_READ:
      {
        if (mounted)
        {
          ...
          log_print_formatted (&log_structure, LOG_URGENCY_DEBUG, "Successfully read %u entries.",
                               directory_entry_read->entries);
          ipc_send (ipc_structure->output_mailbox_id, &message_parameter);
        }
        break;
