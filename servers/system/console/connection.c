...
//      system_call_debug_print_simple ("console: opening console\n");
      ...
      if (*our_console == NULL)
      {
        ...
        (*our_console)->mouse_cursor_visibility = TRUE;
...
static void switch_to_next_console(void);
...
table_of_hot_keys_type table_of_hot_keys[] = 
{
 ...
 /* Right Alt + Right Control + Delete */
 {
...
static bool hot_key_search(keyboard_packet_type *keyboard_packet)
{
  ...
  {
    for( j = 0 ;j < (sizeof(table_of_hot_keys_type) - 6) ; j++ )
    {
      if( (((u8 *)&(table_of_hot_keys[i]))[j]) != (((u8 *)keyboard_packet)[j]) )
      {
        break;
      }
    }
    if( j == (sizeof(table_of_hot_keys_type) - 6) )
    {
...
#if HOT_KEYS
void switch_to_next_console(void)
{
  console_type *new_console = (console_type *) current_console->next;
