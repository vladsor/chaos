/* $Id: console.c,v 1.4 2000/10/03 22:17:51 plundis Exp $ */
...
volatile character_type mouse_cursor = { '#',  1 };
volatile character_type character_under_mouse;
volatile position_type mouse_cursor_position = { 1, 1 };
...
void console_flip (console_type *console)
{
  ...
  if(current_console->mouse_cursor_visibility)
  {
    mouse_cursor_draw (current_console->width, current_console->height, TRUE);
  }
} 
...
int main (void)
{
  ...
  system_call_debug_print_simple ("console starting...\n");
  memory_init ();
