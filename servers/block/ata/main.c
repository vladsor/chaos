/* $Id: main.c,v 1.2 2001/03/11 21:35:59 jojo Exp $ */
                        ...
                        service_type *service)
{
  ...
  bool done = FALSE;
  u32 *data, *message_data;
  unsigned int data_size = 1024 * 256, index;

  memory_allocate ((void **) &data, data_size);
  memory_allocate ((void **) &message_data, 1024);
  ...
  while (!done)
  {
    ...
    message_parameter.data = message_data;
    ...
    switch (message_parameter.message_class)
    {
      case IPC_BLOCK_READ:
      {
        ipc_block_read_type *ipc_block_read = (ipc_block_read_type *) message_data;

        log_print_formatted (&log_structure, LOG_URGENCY_DEBUG,
                             "ipc_block_read - start: %d blocks: %d",
        ...
        {
          /* FIXME: The lowlevel ATA code only supports reading one
             sector at a time, so we will have to read each sector. */

          for (index = 0; index < ipc_block_read->number_of_blocks; index++)
          {
            if (!ata_read_sectors (service->interface, service->device,
                                   ipc_block_read->start_block_number + service->start_sector + index,
                                   1, (char *) data + (index * 512)))
            {
              log_print (&log_structure, LOG_URGENCY_ERROR,
