/* $Id: udp.c,v 1.3 2000/10/18 00:01:35 plundis Exp $ */
   ...
   int length, mailbox_id_type output_mailbox_id __attribute__ ((unused)))
{
  ...
  ipv4_address_to_string (destination_address, ipv4_header->destination_address);
  ...
  if (socket != NULL)
  {
    ...
    packet->length = (length - sizeof (network_ethernet_header_type) -
                      sizeof (ipv4_header_type) - sizeof (udp_header_type));
