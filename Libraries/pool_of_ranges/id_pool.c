...
#define range2 ((range_t *) key2)
    DEBUG_PRINT (DEBUG_LEVEL_INFORMATIVE + 1, 
        ...
        __FILE__, __FUNCTION__, range1->length, key1, range2->length, key2);

    DEBUG_PRINT (DEBUG_LEVEL_INFORMATIVE + 1, 
        "%s::%s Return %i.\n",
        __FILE__, __FUNCTION__, range2->length - range1->length);
...
#define range2 ((range_t *) key2)
    DEBUG_PRINT (DEBUG_LEVEL_INFORMATIVE + 1, 
        "%s::%s {%p}:[%u][%u], {%p}[%u][%u].\n", 
        __FILE__, __FUNCTION__, key1, range1->start, range1->length,
...
//    node_by_index = internal_memory_allocate ();
//    node_by_length = internal_memory_allocate ();
    node_by_index = (range_node_t *) pool->memory.allocate ();
    ...
    node_by_length = (range_node_t *) pool->memory.allocate ();
...
void pool_init (pool_t *pool, range_t range, raw_memory_interface_t memory)
{
    DEBUG_PRINT (DEBUG_LEVEL_INFORMATIVE, 
        "%s::%s with range %u-%u.\n", 
        __FILE__, __FUNCTION__, range.start, range.start + range.length - 1);
...
return_t pool_allocate (pool_t *pool, range_t *range)
{
    ...
    DEBUG_PRINT (DEBUG_LEVEL_INFORMATIVE, 
        "%s::%s range: %p start: %u, length: %u.\n", 
        __FILE__, __FUNCTION__, range, range->start, range->length);
    ...
    current_node = (range_node_t *) avl_tree_search_nearest_node (&pool->free_by_length, 
        (void *) range, &condition);
    ...
    if (is_node_empty (current_node))
    {
        ...
        pool->memory.deallocate (current_node);
        pool->memory.deallocate (current_node2);
    }
    else
    {
