/* $Id: ata.c,v 1.3 2001/03/11 21:35:59 jojo Exp $ */
...
//                         "IRQ %u.", interface->irq);
    ...
    if (ipc_send (interface->mailbox_id, &message_parameter) !=
        IPC_RETURN_SUCCESS)
    {
      log_print (&log_structure, LOG_URGENCY_ERROR, "ipc_send failed.");
      continue;
    }
#endif
                            ...
                            u32 extended_start_sector, u32 offset)
{
  ...
  for (index = 0; index < 4 && partition[index].type != 0; index++)
  {
    log_print_formatted (&log_structure, LOG_URGENCY_INFORMATIVE,
           "  Logical Partition: %s(Type 0x%02X), starts at sector %lu, size: %lu MB.",
	   search_partition_name (partition[index].type),
           partition[index].type,
           partition[index].lba_starting_sector_number,
           (unsigned long int) partition[index].lba_number_of_sectors * 512L 
	                       / 1024L / 1024L);
                           ...
                           u32 sector_number)
{
  ...
  {
    log_print_formatted (&log_structure, LOG_URGENCY_INFORMATIVE,
           "Primary Partition: %s(Type 0x%02X), starts at sector %lu, size: %lu MB. %s",
	   search_partition_name (partition[index].type),
           partition[index].type,
           partition[index].lba_starting_sector_number,
           (unsigned long int) partition[index].lba_number_of_sectors * 512L 
