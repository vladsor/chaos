/* $Id: ipv4.c,v 1.1 2001/02/10 22:58:07 jojo Exp $ */
...
return_type ipv4_init (ipv4_structure_type *ipv4_structure, tag_type *tag)
{
  mailbox_id_type mailbox_id[10];
  unsigned int services = 10;

  ipv4_structure->initialised = TRUE;
  ...
  if (ipc_service_connection_request ((ipc_structure_type *) ipv4_structure) 
      != IPC_RETURN_SUCCESS)
  {
    return IPV4_RETURN_SERVICE_UNAVAILABLE;
  }
...
return_type ipv4_connect 
  (ipv4_structure_type *ipv4_structure, ipv4_connect_type *connect,
...
return_type ipv4_send 
  (ipv4_structure_type *ipv4_structure, ipv4_send_type *send)
{
  message_parameter_type message_parameter;
