/* $Id: system_calls.h,v 1.6 2000/10/18 00:01:34 plundis Exp $ */
...
/* FIXME: Replace the page concept with a block or chunk concept and
   add a kernelfs entry for reading the architectures block or chunk
   size. */
...
static inline return_type system_call_memory_virtual_map
  (process_id_type process_id, u32 virtual_page, u32 physical_page, u32 pages)
{
  return_type return_value;
  ...
  ("\
   ...
   : "g" (pages),
     ...
     "g" (virtual_page),
     "g" (process_id),
     "n" (SYSTEM_CALL_MEMORY_VIRTUAL_MAP << 3));
...
/* FIXME: Remove this when we've reached the top of the IT
   foodchain. */

static inline return_type system_call_debug_print_simple (const char *webwarez)
{
  return_type return_value;
  ...
  ("\
   ...
   : "=a" (return_value)
   : "g" (webwarez), 
     "n" (SYSTEM_CALL_DEBUG_PRINT_SIMPLE << 3));
...
/* Send a message to the given event's queue. */

static inline return_type system_call_event_queue_send
  (event_queue_id_type event_queue_id, 
  event_parameter_type *event_parameter)
{
  ...
  ("\
   ...
   : "g" (event_parameter),
     "g" (event_queue_id),
     "n" (SYSTEM_CALL_EVENT_QUEUE_SEND << 3));

  return return_value;
}

/* Receive a message from the given event's queue. */

static inline return_type system_call_event_queue_receive(
  event_queue_id_type event_queue_id,  
  event_parameter_type *event_parameter)
{
  return_type return_value;
  ...
  ("\
   ...
   : "g" (event_parameter),
     "g" (event_queue_id),
     "n" (SYSTEM_CALL_EVENT_QUEUE_RECEIVE << 3));
   ...
   tag_type *identification)
{
  ...
  /* FIXME: The constraints are a little sub-optimal on this one. */
