 ...
 */
static address_type db_read_address(
	address_type	loc,
	int		short_addr,
	int		regmodrm,
...
{
	if (addrp->is_reg) {
	    debug_print("%s", db_reg[size][addrp->disp]);
	    return;
	}
	...
	if (addrp->base != 0 || addrp->index != 0) {
	    debug_print("%#n", addrp->disp);
	    debug_print("(");
	    if (addrp->base)
		debug_print("%s", addrp->base);
	    if (addrp->index)
		debug_print(",%s,%d", addrp->index, 1<<addrp->ss);
	    debug_print(")");
	} else
	    db_task_printsym((address_type)addrp->disp, DB_STGY_ANY, task);
}
	...
	storm_tss_type *task)
{
	...
	if (mod != 3) {
		...
		case op2(ST,STI):
		    ...
		    debug_print("%s\t%%st,%%st(%d)",name,f_rm(regmodrm));
		    break;
		case op2(STI,ST):
		    name = (fp->f_rrname) ? fp->f_rrname : fp->f_name;
		    debug_print("%s\t%%st(%d),%%st",name, f_rm(regmodrm));
		    break;
		case op1(STI):
		    name = (fp->f_rrname) ? fp->f_rrname : fp->f_name;
		    debug_print("%s\t%%st(%d)",name, f_rm(regmodrm));
		    break;
		case op1(X):
		    debug_print("%s", ((char **)fp->f_rrname)[f_rm(regmodrm)]);
		    break;
		case op1(XA):
		    debug_print("%s\t%%ax",
	...
	storm_tss_type *task)
{
	...
	do {
		...
		case 0xf2:
		    ...
		    break;
		case 0xf3:
		    debug_print("repe ");	/* XXX repe VS rep */
		    break;
		default:
		    prefix = FALSE;
		    break;
	     ...
	     i_mode >>= 8, first = FALSE)
	{
		...
		case R:
		    debug_print("%s", db_reg[size][f_reg(regmodrm)]);
		    break;

		case Rw:
		    debug_print("%s", db_reg[WORD][f_reg(regmodrm)]);
		    break;

		case Ri:
		    debug_print("%s", db_reg[size][f_rm(inst)]);
		    break;

		case S:
		    debug_print("%s", db_seg_reg[f_reg(regmodrm)]);
		    break;

		case Si:
		    debug_print("%s", db_seg_reg[f_reg(inst)]);
		    break;

		case A:
		    debug_print("%s", db_reg[size][0]);	/* acc */
		    break;

		case BX:
		    if (seg)
			debug_print("%s:", seg);
		    debug_print("(%s)", short_addr ? "%bx" : "%ebx");
		    break;
		...
		case SI:
		    if (seg)
			debug_print("%s:", seg);
		    debug_print("(%s)", short_addr ? "%si" : "%esi");
		    break;

		case DI:
		    debug_print("%%es:(%s)", short_addr ? "%di" : "%edi");
		    break;

		case CR:
		    debug_print("%%cr%d", f_reg(regmodrm));
		    break;

		case DR:
		    debug_print("%%dr%d", f_reg(regmodrm));
		    break;

		case TR:
		    debug_print("%%tr%d", f_reg(regmodrm));
		    break;
		...
		case O:
		    ...
		    if (seg)
			debug_print("%s:%#r",seg, displ);
		    else
			db_task_printsym((address_type)displ, DB_STGY_ANY, task);
		    break;
		...
		case Db:
			...
			displ = displ + loc;
		    db_task_printsym((address_type)displ,DB_STGY_XTRN,task);
		    break;
