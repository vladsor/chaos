/* $Id: ipv4.c,v 1.2 2000/10/15 21:15:32 plundis Exp $ */
...
void ipv4_address_to_string (char *string, u32 ipv4_address)
{
    ...
    switch (message_parameter.message_class)
    {
      ...
      case IPC_IPV4_ARP_GET_NUMBER:
      {
        ...
        memory_copy (arp_entry.ethernet_address, entry->ethernet_address,
                     NETWORK_ETHERNET_ADDRESS_LENGTH);
        ipc_send (ipc_structure.output_mailbox_id, &message_parameter);
        break;
      }
    }
...
static bool handle_ethernet (mailbox_id_type mailbox_id)
{
  ...
  message_parameter.length = NETWORK_ETHERNET_ADDRESS_LENGTH;
  ...
  data[0] = NETWORK_ETHERNET_PROTOCOL_IPV4;
  ipc_send (ethernet_structure->output_mailbox_id, &message_parameter);

  /* ...and ARP. */

  data[0] = NETWORK_ETHERNET_PROTOCOL_ARP;
  ipc_send (ethernet_structure->output_mailbox_id, &message_parameter);
  ...
  while (!done)
  {
    ...
    switch (message_parameter.message_class)
    {
      ...
      {
        network_ethernet_header_type *ethernet_header =
          (network_ethernet_header_type *) data;
        
        switch (system_byte_swap_u16 (ethernet_header->protocol_type))
        {
          /* ARP packet. */
          
          case NETWORK_ETHERNET_PROTOCOL_ARP:
          {
          ...
          case NETWORK_ETHERNET_PROTOCOL_IPV4:
          {
            int index;
            ipv4_header_type *ipv4_header =
            ...
            /* It could be to our broadcast or network address. */
...
int main (void)
{
  ...
  if (ipc_service_resolve ("ethernet", mailbox_id, &services, 10, 
  ...
  {
    log_print (&log_structure, LOG_URGENCY_EMERGENCY,
               "Couldn't create network service.");
    return -1;
  }
