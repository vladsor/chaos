...
static char * search_partition_name (uint8_t type)
{
    uint8_t i;
    ...
    uint32_t offset)
{
    ...
    if (block$read (generic, sector_buffer, start_sector + offset, 1) != 
...
return_t scan_for_partitions (handle_t *generic, char *name)
{
    uint8_t *sector_buffer;
    partition_entry_t *partition;
    int index;
    ...
    if (block$read (generic, sector_buffer, 0, 1) != STORM_RETURN_SUCCESS)
    {
        return FALSE;
    }
    ...
    for (index = 0; index < 4 ; index++)
    {
        ...
        if (partition_type == PARTITION_TYPE_NORMAL)
        {
            ...
            partition_block_create (generic, name, 1,
                partition[index].lba_starting_sector_number,
                partition[index].lba_number_of_sectors);
        }
        else if (partition_type == PARTITION_TYPE_EXTENDED)
        {
            ...
            if (check_extended (generic, name, 2, 
