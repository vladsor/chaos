...
return_type kernelfs_entry_read (kernelfs_generic_type *kernelfs_generic)
{
  ...
  switch (kernelfs_generic->kernelfs_class)
  {
    ...
    case KERNELFS_CLASS_CPU_INFO:
    {
      ...
      memory_copy(&kernelfs_cpu_info->info,&CPU,sizeof(full_cpu_info));
      break;
    }
    ...
    case KERNELFS_CLASS_EVENT_QUEUE_AMOUNT:
    {
      unsigned int *event_queues = (unsigned int *) kernelfs_generic;
      *event_queues = event_queue_numbers;
      break;
    }
    
    case KERNELFS_CLASS_EVENT_QUEUE_INFO:
    {
      kernelfs_event_queue_info_type *kernelfs_event_queue_info =
        (kernelfs_event_queue_info_type *) kernelfs_generic;
      unsigned int index = 0;
      unsigned int hash_entry;

      for(hash_entry = 0 ; hash_entry < limit_event_queue_hash_entries ; 
        hash_entry++)
      {
         event_queue_type *node;

         node = event_queue_hash_table[hash_entry];

         while (node != NULL)
         {
           string_copy_max(kernelfs_event_queue_info[index].name, node->name, 
	     MAX_EVENT_QUEUE_NAME_LENGTH - 1);
	   kernelfs_event_queue_info[index].event_queue_id = node->id;
	   kernelfs_event_queue_info[index].number_of_events = node->number_of_events;
	   index++;
           if(index == event_queue_numbers)
	   {
	     hash_entry = limit_event_queue_hash_entries;
	     break;
	   }
    ...
    case KERNELFS_CLASS_IPC_INFO:
    {
      kernelfs_ipc_info_type *kernelfs_ipc_info =
        (kernelfs_ipc_info_type *) kernelfs_generic;

      kernelfs_ipc_info->mailbox_numbers = mailbox_numbers;
      kernelfs_ipc_info->event_queue_numbers = event_queue_numbers;
