...
/* $Id: command.c,v 1.2 2001/02/10 21:23:28 jojo Exp $ */
...
void command_charset (int number_of_arguments, char **argument);
...
void command_hot_key (int number_of_arguments, char **argument);
...
void command_thread_control (int number_of_arguments, char **argument);
...
command_type command[] =
{
  ...
  { "thread_control", "", "", command_thread_control },
...
unsigned int data = 0123;
event_parameter_type event_parameter = {0, sizeof (unsigned int), &data};

void command_event_queue (int number_of_arguments, char **argument)
{
  unsigned int event_queue_id = 0xFFFF;
  return_type return_value;
  unsigned int parameter = 0;
  unsigned int event_class = EVENT_CLASS_NONE;
  ...
  {
    return_value = system_call_event_queue_create (&event_queue_id, 0, 0);
    console_print_formatted (&console_structure, "[%u] new event_queue_id=%u\n",
      return_value,
      event_queue_id);
  }
  else if (!string_compare (argument[1], "destroy"))
  {
    string_to_number (argument[2], &event_queue_id, NULL);
    return_value = system_call_event_queue_destroy (event_queue_id);

    console_print_formatted (&console_structure, "[%u] event_queue_id = %u\n", 
      return_value, event_queue_id);
  }
  else if (!string_compare (argument[1], "register_listener"))
  {
    string_to_number (argument[2], &event_queue_id, NULL);
    return_value = system_call_event_queue_register_listener (event_queue_id, 
                                                              0);
  ...
  else if (!string_compare (argument[1], "get_info"))
  {
    string_to_number (argument[2], &event_queue_id, NULL);
    return_value = 
      system_call_event_queue_get_info (event_queue_id, &event_queue_info);

    console_print_formatted (&console_structure, "[%u] event_queue_id = %u, "
      "length = %u, rejections = %u, listeners = %u\n",
      return_value,
      event_queue_id, event_queue_info.max_number_of_events, 
      event_queue_info.number_of_rejections, 
      event_queue_info.number_of_listeners);
  }
  else if (!string_compare (argument[1], "generate_event"))
  {
    string_to_number (argument[2], &event_queue_id, NULL);
    string_to_number (argument[3], &event_class, NULL);
    string_to_number (argument[4], &parameter, NULL);
    event_parameter.event_class = event_class;
    event_parameter.data = (void *) &parameter;
    return_value = system_call_event_queue_generate_event (event_queue_id, 
                                                           &event_parameter,
							   0);

    console_print_formatted (&console_structure, "[%u]\n", return_value);
  }
  else if (!string_compare (argument[1], "wait_event"))
  {
    string_to_number (argument[2], &event_queue_id, NULL);
    string_to_number (argument[3], &event_class, NULL);
    event_parameter.event_class = event_class;
    return_value = system_call_event_queue_wait_event (event_queue_id, 
                                                       &event_parameter, 
						       0);

    console_print_formatted (&console_structure, "[%u] data = %u\n", 
      return_value, ((unsigned int *) event_parameter.data)[0]);
  }
}



void command_hot_key (int number_of_arguments __attribute__ ((unused)),
                  char **argument __attribute__ ((unused)))
{
  message_parameter_type message_parameter;
  unsigned int number_of_hot_keys, i;
  hot_key_record_type *table_of_hot_keys;
  
  message_parameter.protocol = IPC_PROTOCOL_CONSOLE;
  message_parameter.message_class = IPC_CONSOLE_GET_HOT_KEYS_AMOUNT;
  message_parameter.data = NULL;
  message_parameter.length = 0;
  message_parameter.block = TRUE;

  ipc_send (console_structure.ipc_structure.output_mailbox_id,
            &message_parameter);

  message_parameter.data = &number_of_hot_keys;
  message_parameter.length = sizeof (unsigned int);

  ipc_receive (console_structure.ipc_structure.input_mailbox_id, 
               &message_parameter, NULL);
  
  console_print_formatted (&console_structure, "Number of hot keys: %lu\n",
                           number_of_hot_keys);

  memory_allocate ( (void **) &table_of_hot_keys, number_of_hot_keys * 
                   sizeof (hot_key_record_type));

  message_parameter.protocol = IPC_PROTOCOL_CONSOLE;
  message_parameter.message_class = IPC_CONSOLE_GET_HOT_KEYS_INFO;
  message_parameter.data = NULL;
  message_parameter.length = 0;
  message_parameter.block = TRUE;
  ...
  message_parameter.data = table_of_hot_keys;
  message_parameter.length = number_of_hot_keys * sizeof (hot_key_record_type);
...
*/
  ...
  {
    if (table_of_hot_keys[i].keyboard_packet.left_shift_down)
    {
      console_print (&console_structure, "[Left SHIFT] + ");
    }
    if (table_of_hot_keys[i].keyboard_packet.right_shift_down)
    {
      console_print (&console_structure, "[Right SHIFT] + ");
    }
    if (table_of_hot_keys[i].keyboard_packet.left_alt_down)
    {
      console_print (&console_structure, "[Left Alt] + ");
    }
    if (table_of_hot_keys[i].keyboard_packet.right_alt_down)
    {
      console_print (&console_structure, "[Right Alt] + ");
    }
    if (table_of_hot_keys[i].keyboard_packet.left_control_down)
    {
      console_print (&console_structure, "[Left Ctrl] + ");
    }
    if (table_of_hot_keys[i].keyboard_packet.right_control_down)
    {
      console_print (&console_structure, "[Right Ctrl] + ");
    }
    if (table_of_hot_keys[i].keyboard_packet.has_special_key)
    {
      switch (table_of_hot_keys[i].keyboard_packet.special_key)
      {
        case IPC_KEYBOARD_SPECIAL_KEY_ESCAPE:
	{
          console_print (&console_structure, "[Escape]");
	  break;
	}
	case IPC_KEYBOARD_SPECIAL_KEY_BACK_SPACE:
	{
          console_print (&console_structure, "[Back space]");
	  break;
	}
	case IPC_KEYBOARD_SPECIAL_KEY_TAB:
	{
          console_print (&console_structure, "[Tab]");
	  break;
	}
	case IPC_KEYBOARD_SPECIAL_KEY_ENTER:
	{
          console_print (&console_structure, "[Enter]");
	  break;
	}
/*
...
*/
        case IPC_KEYBOARD_SPECIAL_KEY_F1:
	{
          console_print (&console_structure, "[F1]");
	  break;
	}
	case IPC_KEYBOARD_SPECIAL_KEY_F2:
	{
          console_print (&console_structure, "[F2]");
	  break;
	}
        case IPC_KEYBOARD_SPECIAL_KEY_F3:
	{
          console_print (&console_structure, "[F3]");
	  break;
	}
	case IPC_KEYBOARD_SPECIAL_KEY_F4:
	{
          console_print (&console_structure, "[F4]");
	  break;
	}
/*
  ...
  IPC_KEYBOARD_SPECIAL_KEY_ARROW_UP,
  IPC_KEYBOARD_SPECIAL_KEY_ARROW_DOWN,
  IPC_KEYBOARD_SPECIAL_KEY_ARROW_LEFT,
  IPC_KEYBOARD_SPECIAL_KEY_ARROW_RIGHT,
*/
        case IPC_KEYBOARD_SPECIAL_KEY_DELETE:
	{
          console_print (&console_structure, "[Delete]");
	  break;
	}
      }
    }
    ...
    console_print_formatted (&console_structure, " = \"%s\"\n",
                             table_of_hot_keys[i].description);
  }
}

void command_charset (int number_of_arguments __attribute__ ((unused)),
                      char **argument __attribute__ ((unused)))
{
  console_print_charset (&console_structure);
}
...
void command_pci (int number_of_arguments __attribute__ ((unused)),
                  char *argument[] __attribute__ ((unused)))
{
  unsigned int number_of_devices, i;
  message_parameter_type message_parameter;
...
void command_thread_control (int number_of_arguments, char **argument)
{
  ...
  if (!string_compare (argument[2], "TERMINATE"))
  {
    system_call_thread_control (thread_id, THREAD_TERMINATE, 0);
  }
  if (!string_compare (argument[2], "SLEEP"))
  {
    unsigned int time;
    
    if (string_to_number (argument[3], &time, NULL) != STRING_RETURN_SUCCESS)
    {
      console_print_formatted (&console_structure, 
                               "Invalid time specified!");
      return;
    }
    system_call_thread_control (thread_id, THREAD_SLEEP, time);
  }
  if (!string_compare (argument[2], "BLOCK"))
  {
//    system_call_thread_control (thread_id, THREAD_BLOCK, 0);
  }
  else if (!string_compare (argument[2], "UNBLOCK"))
  {
//    system_call_thread_control (thread_id, THREAD_UNBLOCK, 0);
  }
  else 
  {
    console_print_formatted (&console_structure, 
                  ...
                  char *argument[] __attribute__ ((unused)))
{
  ...
  console_print_formatted (&console_structure, 
                           ...
                           "Process", "Thread", "Uptime", "Memory", "IP",
       ...
       kernelfs_process_info.process_number++)
  {
         ...
         kernelfs_thread_info.thread_number++)
    {
      ...
      console_print_formatted (&console_structure, 
                               ...
                               kernelfs_thread_info.thread_id,
                               (u32) kernelfs_thread_info.uptime,
...
void command_uptime (int number_of_arguments __attribute__ ((unused)),
                     char **argument __attribute__ ((unused)))
{
  kernelfs_time_type uptime;
  uptime.kernelfs_class = KERNELFS_CLASS_UPTIME_INFO;
